document.addEventListener("DOMContentLoaded", function () {
  ...
  const sgpaHeader = document.querySelector(".sgpa-header");
  const tableContainer = document.getElementById("table-container");
  const sgpaOutput = document.getElementById("sgpa-output");
  const cgpaOutput = document.getElementById("cgpa-output"); // Added CGPA output
  const calculateBtn = document.querySelector("#table-container button[onclick='calculateSGPA()']");
  const branchSelect = document.getElementById("branch-select");

  // Array to store SGPA history for CGPA calculation
  let sgpaHistory = [];

  continueBtn.addEventListener("click", function () {
    // FOR Shifting the left section to the left
    sgpaHeader.style.transform = "translateX(-5%)";
    sgpaHeader.style.alignItems = "flex-start";
    sgpaHeader.style.textAlign = "left";
...
{"course_name": "EVOLUTIONARY PSYCHOLOGY","credits": 1  }]
  ...
  // Grade points for each grade (updated to include E grade)
  const gradePoints = {
"A": 10,
"A-": 9,
"B": 8,
...
// Update the course dropdown based on the selected branch
function updateCourseDropdown(branch) {
  const courseSelects = document.querySelectorAll("select[onchange='setCredits(this)']");
  const courses = coursesByBranch[branch] || [];

  courseSelects.forEach((select) => {
    select.innerHTML = `
      <option value="">Select Subject</option>
      ${courses.map(course => `<option value="${course.course_name}">${course.course_name}</option>`).join("")}
    `;
  });}
...
branchSelect.addEventListener("change", function () {
  ...
  updateCourseDropdown(selectedBranch);
});

// Function to set credits based on the selected course
window.setCredits = function (selectElement) {
  ...
  // If course exists, set the credit value
  if (courseData) {
    creditInput.value = courseData.credits;
  } else {
    creditInput.value = ""; // Clear if no valid course is selected
  }
...
window.addRow = function () {
  ...
  newRow.innerHTML = `
      <td>${rowCount}</td>
      <td>
          <select onchange="setCredits(this)">
              <option value="">Select Subject</option>
              ${coursesByBranch[branchSelect.value]
                ?.map(
                  (course) =>
      ...
      <td>
          <input type="number" placeholder="Credits" readonly />
      </td>
      <td>
          <select>
  ...
  const gradeSelect = newRow.querySelector("select:last-of-type");
  const creditInput = newRow.querySelector("input[type='number']");

  gradeSelect.addEventListener("change", validateTable);
  creditInput.addEventListener("input", validateTable);

  // Revalidate the table
  validateTable();
};

// Function to validate the table and enable/disable the Calculate button
function validateTable() {
  ...
  rows.forEach((row) => {
    const courseSelect = row.querySelector("select");
    const gradeSelect = row.querySelector("select:last-of-type");
    const creditInput = row.querySelector("input[type='number']");
    ...
    if (!course || !grade || isNaN(credits) || credits <= 0) {
      isValid = false;
      row.style.backgroundColor = "#ffcccc"; // Highlight invalid rows
    } else {
...
function calculateCGPA() {
  if (sgpaHistory.length === 0) return 0;
  const totalSGPA = sgpaHistory.reduce((sum, sgpa) => sum + sgpa, 0);
  const cgpa = totalSGPA / sgpaHistory.length;
...
window.calculateSGPA = function () {
  ...
  if (totalCredits > 0) {
    ...
    sgpaHistory.push(sgpa); // Store SGPA for CGPA calculation
    const cgpa = calculateCGPA(); // Calculate CGPA

    sgpaOutput.innerText = `Your SGPA is: ${sgpa.toFixed(2)}`;
    cgpaOutput.innerText = `Your CGPA is: ${cgpa}`; // Display CGPA
  } else {
    sgpaOutput.innerText = "Please fill out all fields to calculate SGPA.";
    cgpaOutput.innerText = ""; // Clear CGPA output
  }
};
